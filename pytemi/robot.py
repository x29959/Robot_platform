...
def _on_status(client, userdata, msg):
    ...
    userdata["asr"] = d["asr_result"]

def _on_asr(client, userdata, msg):
    if msg.payload:  # Check if payload is not empty
        try:
            d = json.loads(msg.payload)
            userdata["asr"] = d["asr_result"]
...
class Robot:
    ...
    def __init__(self, mqtt_client, temi_serial, silent=True):
        ...
        # set user data
        self.state = {"locations": [], "battery": {}, "goto": {}, "user": {}, "asr_result": ""}
        ...
        self.client.message_callback_add(
            "temi/{}/status/result".format(temi_serial), _on_asr
        )
    ...
    def backward(self, distance):
        """Forward"""
        if not self.silent:
            print("[CMD] Forward: {} [m]".format(distance))
    ...
    def turn_by(self, angle):
        """Rotate"""
        if not self.silent:
            print("[CMD] Turn By: {} [deg]".format(angle))
        topic = "temi/" + self.id + "/command/move/turn_by"
        payload = json.dumps({"angle": angle})
    ...
    def asr(self, text):
        ...
        if not self.silent:
            print("[CMD] ASR: {}".format(text))

        topic = "temi/" + self.id + "/command/asr"
        payload = json.dumps({"utterance": text})
